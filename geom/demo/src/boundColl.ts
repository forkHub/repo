window.onload = () => {
	...
	let garis2: ha.geom.IGaris = Garis.create(Point.create(0, 0), Point.create(100, 100));
	...
	//garis 2
	let q1: ISprite = Copy(p1);
	Posisi(q1, 110, 10);
	Handle(q1, 8, 8);

	//garis 3
	let q2: ISprite = Copy(p1);
	Posisi(q2, 100, 90);
	Handle(q2, 8, 8);
	...
	function gambarGaris(p1: ISprite, p2: ISprite) {
	...
	function gambarBound(p1: ISprite, p2: ISprite) {
		let ctx: CanvasRenderingContext2D = Kontek();
