class TpsService {
    static url = 'http://localhost:8312/tenant-properties-service/default/properties';
    ...
    static async setProp(nama, value) {
        let conf = ha.comp.ajax.conf();
        ...
        let x = await ha.comp.ajax.send(conf);
        return x;
    }
    static async tambahProp(prop) {
        let conf = ha.comp.ajax.conf();
        conf.header.push({
            key: 'Authorization',
            value: `OIDC_id_token ${tempSession.token}`
