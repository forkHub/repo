window.onload = () => {
    ...
    let badan_ar = [];
    //bola besar
    let kepalaUlar = MuatAnimasi("https://forkhub.github.io/gbr/bola_b.png", 64, 64, true);
    ...
    function buatBadan() {
        let kotak = MuatAnimasi("https://forkhub.github.io/gbr/kotak_2.png", 32, 16);
        Handle(kotak, 0, 8);
        Alpha(kotak, 50);
        return kotak;
    }
    function updateBadan(kepala, badan) {
    ...
    function updateKepala() {
        let x = PosisiX(kepalaUlar);
        let y = PosisiY(kepalaUlar);
        x += vx;
        y += vy;
        if (x + 32 > 600) {
            vx = -buatKec();
    ...
    function updateBadanAr() {
        for (let i = 0; i < badan_ar.length; i++) {
            let badan = badan_ar[i];
            if (i == 0) {
