...
class App {
	...
	async anggotaKlik(view: AnggotaView): Promise<void> {
		...
		if (view.anggota.pasangan && view.anggota.pasangan != '') {
		...
		if (view.anggota.pasangan) {
			console.debug('render pasangan');
			view.namaPasangan.innerHTML = view.anggota.pasangan;
			// view.pasangan.classList.toggle('display-none');
		}
		else {
			view.plus.style.display = 'none';
		}
		...
		if (view.anggota.anak.length == 0) {
			view.hubOrtu.style.display = 'none';
		}
...
class AnggotaView extends ha.comp.BaseComponent {
	...
	get hubOrtu(): HTMLDivElement {
		return this.getEl('div.hub-ortu') as HTMLDivElement;
	}

	get plus(): HTMLDivElement {
		return this.getEl('div.plus') as HTMLDivElement;
	}
