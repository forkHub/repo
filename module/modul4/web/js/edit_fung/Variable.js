...
class Variable {
    tambahVarTbl;
    init() {
        this.tambahVarTbl = Util.getEl('button.tambah-var');
        this.tambahVarTbl.onclick = () => {
            console.log('tambah klik');
            let nama = window.prompt('Nama var: ');
            VariableObj.tambah(new VariableObj(Id.id, nama, fungsiId, 'modul'));
        };
        VariableObj.dihapus = () => {
        ...
        VariableObj.getByParentId((fungsiId)).forEach((item) => {
