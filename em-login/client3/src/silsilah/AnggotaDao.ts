...
namespace ha.sl {
	class AnggotaDao {
		...
		async bacaAnak(anggota: ISlAnggota): Promise<ISlAnggota[]> {
			...
			let h: string = await ha.comp.Util.Ajax2('post', config.nodeServer + ha.comp.Util.getUrl(RouterKOns.p_anggota_id_anak_baca, [anggota.id]), '');
		...
		async bacaPasangan(anggota: ISlAnggota): Promise<ISlAnggota[]> {
			...
			let url: string = config.nodeServer + ha.comp.Util.getUrl(RouterKOns.p_anggota_id_pas_lihat, [anggota.id]);
